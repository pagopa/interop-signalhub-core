...
describe("Pull Signal service", () => {
  ...
  describe("Pull signal with time windows disabled", () => {
  ...
  describe("Pull signal with time windows enabled", () => {
    beforeAll(() => {
      config.featureFlagTimeWindow = true;
      config.timeWindowInSeconds = 5;
    });

    it("Should get an empty signals list if signal has not been persisted yet", async () => {
      const eserviceId = "an-eservice-id";
      const signalPushed = createSignal({ eserviceId });
      await writeSignals([signalPushed], postgresDB, config.signalHubSchema);
      const signalId = 0;
    ...
    it("Should get a one signal after that signal has been persisted (time window passed)", async () => {
      const eserviceId = "an-eservice-id";
      const signalPushed = createSignal({ eserviceId });
      await writeSignals([signalPushed], postgresDB, config.signalHubSchema);
      const signalId = 0;
      ...
      await waitForSeconds(config.timeWindowInSeconds + 1);
