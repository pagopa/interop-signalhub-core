...
export const eServiceProducerRepository = (
  ...
  async eventWasProcessed(streamId: string, version: number): Promise<boolean> {
    try {
      const response = await db.oneOrNone(
        "select event_stream_id, event_version_id from dev_interop.eservice_producer a where a.event_stream_id = $1 AND a.event_version_id = $2",
        [streamId, version]
      );
  ...
  async insert(
    ...
    try {
      await db.none(
        "INSERT INTO DEV_INTEROP.eservice_producer(eservice_id, producer_id,event_stream_id,event_version_id) VALUES ($1, $2, $3, $4)",
